...
public class Main {
    ...
    private final static String[] CHAIR_TYPES = {"Ergonomic", "Executive",
            "Kneeling", "Mesh", "Task"};
    private final static String[] DESK_TYPES = {"Adjustable", "Standing",
            "Traditional"};
    ...
    private final static String HEADER_EXT =
            " - Faculty FrankenFurniture Finder";
    ...
    public static void main(String[] args) {
        ...
        // The order form should be created and used here, using orderResult to
        ...
        //Carter's test code
        LowestCost calculation = new LowestCost(databaseConnection,
                main.category, main.type, main.quantity);
        FurnitureOrder orderResult = calculation.findBestCombination();
        //findBestCombination will create and return an
        ...
        if (orderResult.isFulfilled()) {
            if (gui) {
                graphicalUserInterfaceOutput(orderResult.getFurnitureIDList(),
                        orderResult.getPrice()); // Outputs order details to GUI
            }
            else {
                commandLineOutput(orderResult.getFurnitureIDList(),
                        orderResult.getPrice()); // Outputs order details to
                                                 // console
            }
            
            // Creates order form using orderResult to get all the needed
            // information
            OrderForm.writeOrderForm(orderResult.getTYPE(),
                    orderResult.getCATEGORY(), orderResult.getNUMITEMS(),
                    orderResult.getFurnitureIDList(), orderResult.getPrice());
        }
        else {
            if (gui) {
                graphicalUserInterfaceOutput(
                        orderResult.getManufacturerNameList());
                // Outputs manufacturers to GUI
            }
            else {
                commandLineOutput(orderResult.getManufacturerNameList());
                // Outputs manufacturers to console
            }
        }
    }
    ...
    public void graphicalUserInterfaceInput() {
        try {
            ...
            // String array containing categories
            this.category = (String)JOptionPane.showInputDialog(null,
                    "I am looking for (a)...", "Category" + HEADER_EXT,
                    JOptionPane.INFORMATION_MESSAGE, ICON, categories,
                    categories[0]);
            if (this.category.equals("Chair")) { // Chair is selected
                this.type = (String)JOptionPane.showInputDialog(null,
                        "Select a type of chair:", "Type (Chair)" + HEADER_EXT,
                        JOptionPane.INFORMATION_MESSAGE, ICON, CHAIR_TYPES,
                        CHAIR_TYPES[0]);
            }
            else if (this.category.equals("Desk")) { // Desk is selected
                this.type = (String)JOptionPane.showInputDialog(null,
                        "Select a type of desk:", "Type (Desk)" + HEADER_EXT,
                        JOptionPane.INFORMATION_MESSAGE, ICON, DESK_TYPES,
                        DESK_TYPES[0]);
            }
            else if (this.category.equals("Filing")) { // Filing is selected
                this.type = (String)JOptionPane.showInputDialog(null,
                        "Select a type of filing:", "Type (Filing)"
                                + HEADER_EXT, JOptionPane.INFORMATION_MESSAGE,
                        ICON, FILING_TYPES, FILING_TYPES[0]);
            }
            else { // Lamp is selected
                this.type = (String)JOptionPane.showInputDialog(null,
                        "Select a type of lamp:", "Type (Lamp)" + HEADER_EXT,
                        JOptionPane.INFORMATION_MESSAGE, ICON, LAMP_TYPES,
                        LAMP_TYPES[0]);
            }
        }
        ...
        while (this.quantity < 1) {
            quantityString = (String)JOptionPane.showInputDialog(null,
                    "How many would you like?", "Quantity" + HEADER_EXT,
                    JOptionPane.INFORMATION_MESSAGE, ICON, null, "1");
            if (quantityString == null) {
                System.exit(1); // Exits if user clicks "Cancel" button
            }
            try {
                this.quantity = Integer.parseUnsignedInt(quantityString);
                // parses inputted String to int
            }
            catch (NumberFormatException e){
                if (quantityString.length() == 0) {
                    // Error handling for no input
                    JOptionPane.showMessageDialog(null,
                            "At least one item must be requested.", "Error"
                                    + HEADER_EXT, JOptionPane.ERROR_MESSAGE,
                            ERRORICON);
                }
                else {
                    // Error handling for invalid number
                    JOptionPane.showMessageDialog(null, quantityString +
                            " is not a valid number of items.", "Error"
                            + HEADER_EXT, JOptionPane.ERROR_MESSAGE, ERRORICON);
                }
            }
            if (quantity == 0) {
     ...
     */
    public static void graphicalUserInterfaceOutput(
            LinkedList<String> furnitureIDList, int price) {
        String message = "";
        ...
        if (furnitureIDList.size() == 1) {
            ...
            message = message + furnitureIDList.get(0) + " and "  +
                    furnitureIDList.get(1) + " for $" + price + ".";
        }
        else if (furnitureIDList.size() > 2) {
            for (int i = 0; i < furnitureIDList.size() - 1; i++) {
                message = message + furnitureIDList.get(i) + ", ";
            }
            message = message + "and "  + furnitureIDList.get(
                    furnitureIDList.size() - 1) + " for $" + price + ".";
        }
        // Displays message
        JOptionPane.showMessageDialog(null, message, "Item(s) Found" +
                HEADER_EXT, JOptionPane.INFORMATION_MESSAGE, ICON);
    }
     ...
     */
    public static void graphicalUserInterfaceOutput(
            LinkedList<String> manufacturerList) {
        String message = "";
        message = message +
        ...
        if (manufacturerList.size() == 1) {
            ...
            message = message + "s are " + manufacturerList.get(0) + " and "  +
            ...
            message = message + "and " + manufacturerList.get(
                    manufacturerList.size() - 1) + ".";
        }
        // Displays message
        JOptionPane.showMessageDialog(null, message, "Item(s) Not Found" +
                HEADER_EXT, JOptionPane.ERROR_MESSAGE, ERRORICON);
    }
    ...
    public void commandLineInput(String args[]) {
        ...
        category = args[0].substring(0, 1).toUpperCase() +
        ...
        if (args.length == 4) { // handles possible space when inputting type
            // (e.g. Swing Arm)
            ...
            for (int i = 1; i < args[2].length(); i++) {
                if (args[2].charAt(i) == ' ')  {
                    ...
                    type = type +
                            String.valueOf(args[2].charAt(i)).toUpperCase();
                    upperCaseNext = false;
                }
                else {
                    type = type +
                            String.valueOf(args[2].charAt(i)).toLowerCase();
                }
            }
        }
        ...
        if (category.equals("Chair")) {
            ...
            System.out.println(category +
                    " is not a valid furniture category. Please try again.");
            System.exit(1);
        }
        if (!validType) {
            // Error handling for invalid type
            System.out.println(type + " is not a valid " +
                    category.toLowerCase() + " type. Please try again.");
            System.exit(1);
        }
        try {
            ...
            System.out.println(args[args.length - 1] +
                    " is not a valid number of items. Please try again.");
            System.exit(1);
        }
        if (quantity == 0) {
            // Error handling for input of zero
            System.out.println(
                    "At least one item must be requested. Please try again.");
            System.exit(1);
        }
    }
     ...
     */
    public static void commandLineOutput(LinkedList<String> furnitureIDList,
                                         int price) {
        ...
        if (furnitureIDList.size() == 1) {
            ...
            System.out.println(furnitureIDList.get(0) + " and "  +
                    furnitureIDList.get(1) + " for $" + price + ".");
        }
        else if (furnitureIDList.size() > 2) {
            for (int i = 0; i < furnitureIDList.size() - 1; i++) {
            ...
            System.out.println("and "  + furnitureIDList.get(
                    furnitureIDList.size() - 1) + " for $" + price + ".");
        }
    }
    ...
    public static void commandLineOutput(LinkedList<String> manufacturerList) {
        System.out.print("Order cannot be fulfilled based on current " +
                "inventory. Suggested manufacturer");
        if (manufacturerList.size() == 1) {
            System.out.println(" is " + manufacturerList.get(0) + ".");
        }
        else if (manufacturerList.size() == 2) {
            System.out.println("s are " + manufacturerList.get(0) + " and "  +
                    manufacturerList.get(1) + ".");
        }
        else if (manufacturerList.size() > 2) {
            ...
            System.out.println("and "  + manufacturerList.get(
                    manufacturerList.size() - 1) + ".");
        }
    }
}